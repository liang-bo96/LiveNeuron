...
class BrainPlotly2DViz:
    ...
    def _load_sample_data(self, region: Optional[str] = None) -> None:
        ...
        if EELBRAIN_AVAILABLE and region is not None:
            ...
            try:
                ...
                warnings.warn(
                    f"Could not load eelbrain sample data: {e}. Using synthetic data."
                )

        # Use synthetic sample data
        sample_data = create_sample_brain_data(
            n_sources=200, n_times=50, has_vector_data=True, random_seed=42
        )
    ...
    def _create_plotly_brain_projection(
        self, time_idx: int, view_name: str
    ) -> go.Figure:
        ...
        if len(active_coords) > 0:
            ...
            zmin, zmax = 0, (
                np.max(active_magnitudes) if len(active_magnitudes) > 0 else 1
            )
            ...
            if has_vector_data and self.arrow_threshold != "none":
                ...
                if np.any(arrow_mask):
                    arrow_scale = self._calculate_arrow_scale(
                        u_vectors[arrow_mask], v_vectors[arrow_mask]
                    )
            ...
            if (
                hasattr(self, "selected_source_idx")
                and self.selected_source_idx is not None
            ):
    ...
    def _calculate_arrow_scale(
        self, u_vectors: np.ndarray, v_vectors: np.ndarray
    ) -> float:
    ...
    def create_butterfly_plot(self) -> go.Figure:
        ...
        if not self.show_max_only:
            ...
            for i in range(
                min(50, magnitudes.shape[0])
            ):  # Limit to 50 traces for performance
                fig.add_trace(
                    go.Scatter(
                        x=self.time_values,
                        y=magnitudes[i, :],
    ...
    def run(
        self, port: int = 8050, debug: bool = False, host: str = "127.0.0.1"
    ) -> None:
        ...
        app.layout = html.Div(
            [
                html.H1(
                    "Interactive 2D Brain Visualization",
                ...
                html.Div(
                    [
                        ...
                        dcc.Slider(
                            ...
                            marks={
                                i: f"{self.time_values[i]:.3f}s"
                                for i in range(
                                    0,
                                    len(self.time_values),
                                    max(1, len(self.time_values) // 10),
                                )
                            },
                        ...
                        html.Div(
                            [
                                html.Div(
                                    [
                                        html.H4(
                                            "Axial View", style={"textAlign": "center"}
                                        ),
                ...
                html.Div(
                    id="status-display", style={"textAlign": "center", "marginTop": 20}
                ),
            ]
        )
        ...
        def update_brain_projections(
            time_idx: int | None,
        ) -> tuple[go.Figure, go.Figure, go.Figure, int]:
            if time_idx is None:
                time_idx = 0
        ...
        @app.callback(
            Output("butterfly-plot", "figure"), [Input("selected-time-idx", "children")]
        )
    ...
    def show_in_jupyter(
        self, time_idx: int = 0, width: int = 1200, height: int = 800
    ) -> None:
